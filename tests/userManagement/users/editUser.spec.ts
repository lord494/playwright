...
test('Korisnik moze da edituje usera popunjavanjem svih polja', async ({ page }) => {
    ...
    await invite.editData(invite.phoneField, Constants.adminPhone);
    ...
    await invite.editData(invite.passwordField, Constants.password);
...
test('Provjera da je permisija vidljiva nakon dodavanja usera', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await user.searchUser(user.searchInputField, email);
    await user.pencilIcon.nth(1).waitFor({ state: 'hidden', timeout: 10000 });
